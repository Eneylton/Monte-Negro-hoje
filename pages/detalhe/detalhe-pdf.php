...
$entregadores = Entregador::getList('t.id as id,e.cod_id as codigo,t.nome as entregador,e.data as data_entrega,dev.data as data_devolucao,
                                     e.qtd as entrega,dev.qtd as devolucao,l.receber_id AS receber_id',
                                     'logisticas AS l INNER JOIN entrega AS e ON (l.id = e.logisticas_id)
                                      left JOIN devolucao AS dev ON (l.id = dev.logisticas_id) INNER JOIN
                                      entregadores AS t ON (l.entregadores_id = t.id) INNER JOIN receber AS r ON (l.receber_id = r.id)',
                                      'l.receber_id = '.$_GET['id']);
...
foreach ($entregadores as $item) {
    ...
    $res .= '   <tr>
                        ...
                        <td style="text-align:left;width:231px">' . date('d/m/Y à\s H:i:s', strtotime($item->data_entrega)) . '</td>
                        <td style="text-align:left;width:231px">' . date('d/m/Y à\s H:i:s', strtotime($item->data_devolucao)) . '</td>
